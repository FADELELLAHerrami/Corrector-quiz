...
const AjouteQuiz = async (req, res) => {
    ...
    const {name,startDate,endDate,students,attemptsAllowed,attempts,questions} = req.body;
    console.log({name,startDate,endDate,students,attemptsAllowed,attempts,questions});
    const newQuiz = await quizModel.create({name,startDate,endDate,students,attemptsAllowed,attempts,questions});
    res.json(newQuiz);
  };
...
const updateQuiz = async (req,res)=>{
    const quizId = req.params;
    const {name,startDate,endDate,students,attemptsAllowed,attempts,questions} = req.body;
    const updatedQuiz = await quizModel.findByIdAndUpdate(quizId,{name,startDate,endDate,students,attemptsAllowed,attempts,questions});
    res.json(updatedQuiz);
};
